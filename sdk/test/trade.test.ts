...
describe('Trade', () => {
  ...
  const pair_weth_0 = new Pair(
    new TokenAmount(WETH[ChainId.SEPOLIA], JSBI.BigInt(1000)),
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for DEV currency input', () => {
      ...
      expect(result[0].route.path).toEqual([WETH[ChainId.SEPOLIA], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(DEV)
      expect(result[1].route.path).toEqual([WETH[ChainId.SEPOLIA], token0, token3])
    ...
    it('works for DEV currency output', () => {
      ...
      expect(result[0].route.path).toEqual([token3, token0, WETH[ChainId.SEPOLIA]])
      expect(result[0].outputAmount.currency).toEqual(DEV)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WETH[ChainId.SEPOLIA]])
